...
func Test_Wrapper_ConcatNew(t *testing.T) {
	Convey("ConcatNew chain methods", t, func() {
		...
		So(c.MsgRefOne("msg", "r1"), ShouldNotBeNil)
		So(c.MsgRefTwo("msg", "r1", "r2"), ShouldNotBeNil)
		So(c.MsgRefsOnly("msg", "r1"), ShouldNotBeNil)
		So(c.Msg("msg"), ShouldNotBeNil)
		So(c.MsgUsingStackSkip(0, "msg"), ShouldNotBeNil)
		So(c.Error(errtype.Generic, errors.New("inner")), ShouldNotBeNil)
		So(c.ErrorUsingStackSkip(0, errtype.Generic, errors.New("inner")), ShouldNotBeNil)
		So(c.Errors(errtype.Generic, errors.New("a"), errors.New("b")), ShouldNotBeNil)
