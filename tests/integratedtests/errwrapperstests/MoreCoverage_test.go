...
func Test_Constructors(t *testing.T) {
	...
	Convey("NewUsingErrorWrappers + clone + ptr", t, func() {
		...
		So(errwrappers.NewUsingErrorWrappersClone(w1, w2).Count(), ShouldEqual, 2)
...
func Test_MutexCollection_All(t *testing.T) {
	Convey("MutexCollection constructors + methods", t, func() {
		...
		So(mc.IsEmpty(), ShouldBeTrue)
		So(mc.HasError(), ShouldBeFalse)
		So(mc.Length(), ShouldEqual, 0)
		So(mc.IsSuccess(), ShouldBeTrue)
		So(mc.IsValid(), ShouldBeTrue)
		So(mc.IsFailed(), ShouldBeFalse)
		...
		So(mc.HasError(), ShouldBeTrue)
		So(mc.Length(), ShouldEqual, 2)
		So(mc.IsEmpty(), ShouldBeFalse)
		So(mc.String(), ShouldNotBeBlank)
		So(mc.ToStringLock("\n"), ShouldNotBeBlank)
		So(mc.ToStringsLock(), ShouldNotBeEmpty)
