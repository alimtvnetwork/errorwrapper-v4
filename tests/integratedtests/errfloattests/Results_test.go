...
func Test_Errfloat_Results_Basics(t *testing.T) {
	Convey("Nil receiver", t, func() {
		var r *errfloat.Results
	...
	Convey("Empty slice", t, func() {
		r := &errfloat.Results{Values: []float32{}, ErrorWrapper: nil}
		So(r.IsEmpty(), ShouldBeTrue)
		So(r.HasAnyItem(), ShouldBeFalse)
		So(r.Length(), ShouldEqual, 0)
	})
	...
	Convey("Non-empty slice without error", t, func() {
		r := &errfloat.Results{Values: []float32{1.1, 2.2}, ErrorWrapper: nil}
	...
	Convey("Dispose nils values and wrapper", t, func() {
		...
		r := &errfloat.Results{Values: []float32{1.1, 2.2}, ErrorWrapper: w}
		r.Dispose()
...
func Test_Errfloat_Results_JSON(t *testing.T) {
