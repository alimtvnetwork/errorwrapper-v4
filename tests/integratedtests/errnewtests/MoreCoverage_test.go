...
func Test_MoreCoverage_Error(t *testing.T) {
	Convey("Error creator family", t, func() {
		...
		So(errnew.Error.Default(err), ShouldNotBeNil)
		So(errnew.Error.UsingStackSkip(0, errtype.IO, err).HasError(), ShouldBeTrue)
		So(errnew.Error.DefaultUsingStackSkip(0, err), ShouldNotBeNil)
...
func Test_MoreCoverage_Path(t *testing.T) {
	Convey("Path creator family", t, func() {
		...
		So(errnew.Path.File("/a"), ShouldNotBeNil)
		So(errnew.Path.FileContentIssue("/a"), ShouldNotBeNil)
...
func Test_MoreCoverage_Null(t *testing.T) {
	Convey("Null creator family", t, func() {
		...
		So(errnew.Null.UsingStackSkip(0, nil), ShouldNotBeNil)
		So(errnew.Null.Message("hello"), ShouldNotBeNil)
		So(errnew.Null.Message(""), ShouldBeNil)
		So(errnew.Null.WithMessage(nil, "ctx"), ShouldNotBeNil)
		So(errnew.Null.Error(nil, errors.New("e")), ShouldNotBeNil)
		So(errnew.Null.Error("x", errors.New("e")), ShouldBeNil)
		So(errnew.Null.ErrorWithMessage(nil, errors.New("e"), "ctx"), ShouldNotBeNil)
		So(errnew.Null.OrWrapper(nil, errnew.Messages.Single(errtype.IO, "fallback")), ShouldNotBeNil)
		So(errnew.Null.OrWrapper("notnull", errnew.Messages.Single(errtype.IO, "fallback")), ShouldBeNil)
...
func Test_MoreCoverage_Refs(t *testing.T) {
	Convey("Refs creator family", t, func() {
		...
		So(errnew.Refs.Messages(errtype.IO, []ref.Value{r}, "a", "b"), ShouldNotBeNil)
		So(errnew.Refs.MessagesUsingStackSkip(0, errtype.IO, []ref.Value{r}, "a"), ShouldNotBeNil)
		So(errnew.Refs.MessagesUsingJoiner(errtype.IO, " | ", []ref.Value{r}, "a", "b"), ShouldNotBeNil)
		So(errnew.Refs.MessagesUsingJoinerStackSkip(0, errtype.IO, " | ", []ref.Value{r}, "a"), ShouldNotBeNil)
